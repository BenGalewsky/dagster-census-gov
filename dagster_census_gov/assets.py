        ...
        with tempfile.SpooledTemporaryFile(
            ...
            with ZipFile(tmp) as zip_file:
                ...
                for file_info in zip_file.infolist():
                    ...
                    s3_key = f"{s3_prefix.rstrip('/')}/{file_info.filename.replace('.txt', '.parquet')}"  # NOQA E501
...
def tiger_shapefile_asset(context: AssetExecutionContext,
                          ...
                          s3: S3Resource) -> None:
    url = f"https://www2.census.gov/geo/tiger/TIGER{config.year}/{config.geographic_entity}/" # NOQA E501
...
def save_expanded_gazetteer(url: str,
    ...
    try:
        ...
        for file in zipped_files(url, config.s3_prefix, log):
            s3_key = f"{EnvVar('DEST_BUCKET').get_value()}/{config.s3_prefix.rstrip('/')}/{config.year}/{config.geographic_entity}"  # NOQA E501
...
def gazetteer(context: AssetExecutionContext,
              config: GazetterConfig,
              s3: S3Resource) -> None:
    url = f"https://www2.census.gov/geo/docs/maps-data/data/gazetteer/{config.year}_Gazetteer/{config.year}_Gaz_{config.geographic_entity.lower()}_national.zip"  # NOQA E501
